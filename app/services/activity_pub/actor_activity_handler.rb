class ActivityPub::ActorActivityHandler
  def self.handle_create_activity(activity_hash_or_id)
    Rails.logger.info "ActivityPub::ActorActivityHandler handle_create_activity: #{activity_hash_or_id}"
    handle_activity(activity_hash_or_id, "Create")
  end

  def self.handle_update_activity(activity_hash_or_id)
    Rails.logger.info "ActivityPub::ActorActivityHandler handle_update_activity: #{activity_hash_or_id}"
    handle_activity(activity_hash_or_id, "Update")
  end

  def self.handle_activity(activity_hash_or_id, action)
    Rails.logger.info "ActivityPub::ActorActivityHandler handle_activity: #{activity_hash_or_id}, #{action}"
  ...
  def self.get_actor(activity)
    Rails.logger.info "ActivityPub::ActorActivityHandler get_actor: #{activity}"
  ...
  def self.actor_object_attributes(activity)
    Rails.logger.info "ActivityPub::ActorActivityHandler actor_object_attributes: #{activity}"
  ...
  def self.server_and_port(string)
    Rails.logger.info "ActivityPub::ActorActivityHandler server_and_port: #{string}"
