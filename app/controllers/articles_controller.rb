  ...
  def prepare_readability_output_for_tts(readability_output)
    ...
    def process_string(item)
      item
        ...
        # .gsub(/\b([A-Z]{2,4})(s?)\b/) do    # Add spaces between letters in short acronyms (2-4 chars, e.g., USA -> U S A, ICBMs -> I C B M's)
        ...
        # .gsub(/"(.*?)"/) { |_match| ", quote, #{::Regexp.last_match(1)}, end quote, " }  # Make quotations explicitly readable
        .gsub(/"(.*?)"/) { |_match| ", quote, #{::Regexp.last_match(1)}," }  # Make quotations explicitly readable
        .gsub(/"(.*?)"/) { |_match| ", quote, #{::Regexp.last_match(1)}," }  # Make quotations explicitly readable
        .strip
        .gsub(/(?<![.!?])$/, ".")
    end
