// @mintplex-labs/piper-tts-web@1.0.4 downloaded from https://unpkg.com/@mintplex-labs/piper-tts-web@1.0.4/dist/piper-tts-web.js
...
const ONNX_BASE =
  "https://cdnjs.cloudflare.com/ajax/libs/onnxruntime-web/1.20.1/";
// const ONNX_BASE = "/wasm/";
const WASM_BASE =
...
const _TtsSession = class _TtsSession {
  constructor({ voiceId, progress, logger, wasmPaths }) {
    ...
    (_a = __privateGet(this, _logger)) == null
      ...
      : _a.call(
          this,
          `Loaded WASMPaths at: ${JSON.stringify(
  ...
  async init() {
    ...
    __privateGet(this, _ort).env.wasm.wasmPaths = __privateGet(
      this,
      _wasmPaths
    ).onnxWasm;
    ...
    __privateSet(
      this,
      _ortSession,
      await __privateGet(this, _ort).InferenceSession.create(
  ...
  async predict(text) {
    ...
    const phonemeIds = await new Promise(async (resolve) => {
      const module = await __privateGet(this, _createPiperPhonemize).call(
        this,
        {
          print: (data) => {
    ...
    const feeds = {
      input: new (__privateGet(this, _ort).Tensor)("int64", phonemeIds, [
        1,
        phonemeIds.length,
      ]),
      input_lengths: new (__privateGet(this, _ort).Tensor)("int64", [
        phonemeIds.length,
      ]),
...
async function download(voiceId, callback) {
  ...
  await Promise.all(
    urls.map(async (url) => {
      writeBlob(
        url,
        await fetchBlob(url, url.endsWith(".onnx") ? callback : void 0)
      );
    })
  );
}
...
async function voices() {
  try {
    ...
    console.log(
      `Could not fetch voices.json remote ${HF_BASE}. Fetching local`
    );
