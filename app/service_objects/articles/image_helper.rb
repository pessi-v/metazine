  ...
  class ImageHelper
    def self.compare_and_update_article_images(article)
      ...
      begin
        ...
        File.open(temp_article_image.path, 'wb') do |file|
          file.write(URI.parse(image_url).open.read)
        end
        
        File.open(temp_content_image.path, 'wb') do |file|
          file.write(URI.parse(content_image_url).open.read)
        end
        ...
        Rails.logger.error("Error comparing images: #{e.message}")
      ensure
        # Clean up temp files
